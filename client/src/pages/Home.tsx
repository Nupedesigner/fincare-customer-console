...
import {
  ...
  Loader2, MoreHorizontal, PanelLeftClose, Search, Settings2, ShieldCheck, SlidersHorizontal,
...
function ConversationDetail({ back }: { back: () => void }) {
  const [acceptState, setAcceptState] = useState<"idle" | "processing" | "accepted">("idle");
  const [reauthState, setReauthState] = useState<"not-requested" | "requested" | "verified">("not-requested");
  const [draft, setDraft] = useState("Hello Ibrahim, I’m Damilola from Qorebank Support. I can confirm that the transfer was not completed and your account was not debited. Please try again in a few minutes. If the issue continues, I can help you review the next steps.");
  const [agentNotes, setAgentNotes] = useState("Transfer lookup confirms no debit. Customer advised to retry after a short interval.");
  const [disposition, setDisposition] = useState("Resolved — retry advised");
  ...
  const requestReauth = () => { setReauthState("requested"); toast("Secure re-authentication request sent", { description: "The customer will complete verification in the existing Qorebank web or mobile session. No code is exposed to the agent." }); };
  const accepted = acceptState === "accepted";
  const acceptHandoff = () => { setAcceptState("processing"); window.setTimeout(() => { setAcceptState("accepted"); toast("Hand-off accepted", { description: "Damilola O. now owns this case. The customer will see that a human support agent has joined." }); }, 900); };
  const sendReply = () => { setReplySent(true); toast("Human response sent", { description: `Reply recorded with disposition: ${disposition}.` }); };
  const exportAuditPdf = () => {
    ...
    let y = 54;
    const addLines = (text: string, size = 10, weight: "normal" | "bold" = "normal", gap = 6) => {
      pdf.setFont("helvetica", weight);
      pdf.setFontSize(size);
      const lines = pdf.splitTextToSize(text, usableWidth);
      if (y + lines.length * (size + 3) > 782) { pdf.addPage(); y = 54; }
      pdf.text(lines, margin, y);
      y += lines.length * (size + 3) + gap;
    };
    pdf.setFillColor(7, 26, 53); pdf.rect(0, 0, pageWidth, 86, "F");
    pdf.setTextColor(255, 255, 255); pdf.setFont("helvetica", "bold"); pdf.setFontSize(18); pdf.text("FinCare Audit Record", margin, 42);
    pdf.setFontSize(9); pdf.text("Conversation CNV-928416 · Qorebank Production", margin, 63);
    pdf.setTextColor(7, 26, 53); y = 116;
    addLines("AUDIT SCOPE", 9, "bold");
    addLines(`Generated ${new Date().toLocaleString()} · Channel: secure web conversation · Customer: Ibrahim O. · CUS-001764 · Account: Savings · **** 4821`);
    addLines("FINCARE AI HAND-OFF BRIEF", 11, "bold");
    addLines("Why this was handed off: The customer asked about a transfer failure after FinCare had completed the available guidance. The transfer lookup found a beneficiary-bank timeout before confirmation, with no debit recorded.");
    addLines("Customer need: Confirm outcome and advise the safe next step.");
    addLines("Recommended human response: Confirm no debit, invite a retry, and request step-up verification only if account-specific action is needed.");
    addLines("CONVERSATION HISTORY", 11, "bold");
    messages.forEach(([speaker, message], index) => addLines(`${String(index + 1).padStart(2, "0")} · ${speaker}: ${message}`));
    if (replySent) addLines(`Human Agent · Damilola O.: ${draft}`);
    addLines("RESOLUTION TRACKING", 11, "bold");
    addLines(`Hand-off status: ${accepted ? "Accepted by Damilola O." : "Awaiting human acceptance"}`);
    addLines(`Disposition: ${disposition}`);
    addLines(`Private agent notes: ${agentNotes}`);
    addLines(`Secure-session status: ${reauthState === "verified" ? "Customer re-authentication confirmed" : reauthState === "requested" ? "Customer re-authentication requested" : "Not requested"}`);
    addLines("This client-side export is a prototype audit record. Production exports should be generated by an audited server-side service with immutable event IDs, access controls, retention rules, and a cryptographic integrity record.", 8, "normal", 0);
    pdf.save("fincare-CNV-928416-audit-record.pdf");
    toast("Audit PDF downloaded", { description: "The conversation history and FinCare hand-off brief have been exported." });
  };
  const messageAvatar = (speaker: string) => speaker === "FinCare AI" ? <img src={iconLogo} alt="FinCare AI" className="h-5 w-5" /> : speaker === "System" ? <Zap className="h-3.5 w-3.5" /> : speaker === "Human Agent" ? "DO" : "IO";
  return <div className="app-enter space-y-5">
    <button onClick={back} className="inline-flex items-center gap-2 text-xs font-semibold text-[#55739e] hover:text-[#0866f5]"><ArrowLeft className="h-4 w-4" />Back to conversations</button>
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[11px] font-semibold uppercase tracking-[.11em] text-[#0866f5]">CONVERSATION INSPECTION</p><h1 className="mt-1 text-2xl font-bold tracking-[-.035em] text-[#071a35]">CNV-928416</h1><p className="mt-1 text-sm text-slate-600">Ibrahim O. · CUS-001764 · Web chat · Started today at 09:37</p></div><div className="flex items-center gap-2"><button onClick={exportAuditPdf} className="inline-flex items-center gap-2 rounded-lg border border-[#c9dcf5] bg-white px-3 py-2 text-xs font-semibold text-[#0866f5] hover:bg-[#f4f8ff]"><Download className="h-3.5 w-3.5" />Export audit PDF</button><Status value="Escalated" /></div></div>
    <section className="grid gap-5 xl:grid-cols-[210px_minmax(620px,1.55fr)_250px]">
      <aside className="h-fit rounded-xl border border-[#dbe5f4] bg-white p-5 shadow-[0_1px_2px_rgba(7,26,53,.025)]"><p className="metric-label">CUSTOMER CONTEXT</p><div className="mt-5 flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#eaf3ff] text-sm font-bold text-[#0866f5]">IO</span><div><p className="text-sm font-bold text-[#071a35]">Ibrahim O.</p><p className="text-[11px] text-slate-500">CUS-001764</p></div></div><dl className="mt-6 space-y-4 text-xs"><div><dt className="text-slate-500">Account type</dt><dd className="mt-1 font-semibold text-[#253b5a]">Savings · **** 4821</dd></div><div><dt className="text-slate-500">Support history</dt><dd className="mt-1 font-semibold text-[#253b5a]">8 conversations · 2 escalations</dd></div><div><dt className="text-slate-500">Last CSAT</dt><dd className="mt-1 font-semibold text-[#253b5a]">3.5 / 5</dd></div><div><dt className="text-slate-500">Current channel</dt><dd className="mt-1 font-semibold text-[#253b5a]">Secure web conversation</dd></div></dl><button onClick={() => buttonToast("Customer profile")} className="mt-6 w-full rounded-lg border border-[#d5e2f4] py-2 text-xs font-semibold text-[#0866f5] hover:bg-[#f4f8ff]">View customer profile</button></aside>
      <article className="fincare-card overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#eef3fa] px-5 py-4"><div><p className="text-sm font-bold text-[#071a35]">Conversation timeline</p><p className="mt-1 text-xs text-slate-500">Transfer Failure · 5m 32s</p></div>{acceptState === "processing" ? <span className="status-chip bg-[#e6f8fd] text-[#078bb5]"><Loader2 className="h-3 w-3 animate-spin" />Assigning this case...</span> : accepted ? <span className="status-chip bg-[#eaf3ff] text-[#0866f5]"><Users className="h-3 w-3" />Damilola owns this case</span> : <button onClick={acceptHandoff} className="rounded-lg bg-[#0866f5] px-3 py-2 text-xs font-semibold text-white hover:bg-[#075ddd] active:scale-[.97]">Accept hand-off</button>}</div>
        <div className="space-y-5 p-5">
          <section className="rounded-xl border border-[#cfe4fa] bg-[#f4f8ff] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="flex items-center gap-2"><img src={iconLogo} alt="FinCare AI" className="h-7 w-7" /><div><p className="text-xs font-bold text-[#071a35]">FinCare hand-off brief</p><p className="text-[10px] font-medium text-[#078bb5]">AI-generated context · Review before responding</p></div></div><span className="status-chip bg-[#e6f8fd] text-[#078bb5]"><Bot className="h-3 w-3" />96% confidence</span></div><p className="mt-3 text-xs leading-5 text-[#294460]"><strong className="font-semibold text-[#071a35]">Why this was handed off:</strong> The customer asked about a transfer failure after FinCare had completed the available guidance. The transfer lookup found a beneficiary-bank timeout before confirmation, with no debit recorded.</p><div className="mt-3 grid gap-2 sm:grid-cols-2"><div className="rounded-lg bg-white p-3"><p className="text-[10px] font-semibold uppercase tracking-[.075em] text-slate-500">Customer need</p><p className="mt-1 text-[11px] font-semibold text-[#253b5a]">Confirm outcome and advise the safe next step.</p></div><div className="rounded-lg bg-white p-3"><p className="text-[10px] font-semibold uppercase tracking-[.075em] text-slate-500">Recommended human response</p><p className="mt-1 text-[11px] font-semibold text-[#253b5a]">Confirm no debit, invite a retry, and request step-up verification only if account-specific action is needed.</p></div></div><p className="mt-3 text-[10px] leading-4 text-slate-500">No fraud, PII, or policy concern was detected. Do not expose account details or take an account action until the secure-session status permits it.</p></section>
          <div className="space-y-5">{messages.map(([speaker, message], i) => <div key={`${speaker}-${i}`} className="flex gap-3"><span className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-[10px] font-bold ${speaker === "FinCare AI" ? "bg-[#e6f8fd] text-[#078bb5]" : speaker === "System" ? "bg-[#f1f5f9] text-[#64748b]" : "bg-[#eaf3ff] text-[#0866f5]"}`}>{messageAvatar(speaker)}</span><div><div className="flex items-baseline gap-2"><p className="text-xs font-semibold text-[#253b5a]">{speaker}</p><span className="text-[10px] text-slate-400">09:{37 + i}</span></div><p className={`mt-1 max-w-[650px] rounded-lg px-3 py-2.5 text-xs leading-5 ${speaker === "FinCare AI" ? "bg-[#f4f8ff] text-[#28415f]" : speaker === "System" ? "bg-[#f8fafc] text-slate-500" : "bg-[#eef5ff] text-[#28415f]"}`}>{message}</p></div></div>)}{replySent && <div className="flex gap-3"><span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[#eaf3ff] text-[10px] font-bold text-[#0866f5]">DO</span><div><div className="flex items-baseline gap-2"><p className="text-xs font-semibold text-[#253b5a]">Human Agent · Damilola O.</p><span className="text-[10px] text-slate-400">Now</span></div><p className="mt-1 max-w-[650px] rounded-lg bg-[#edf5ff] px-3 py-2.5 text-xs leading-5 text-[#28415f]">{draft}</p></div></div>}</div>
          <section className="rounded-xl border border-[#dbe5f4] bg-white"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#eef3fa] px-4 py-3"><div><p className="text-xs font-bold text-[#071a35]">Human agent response</p><p className="mt-0.5 text-[10px] text-slate-500">Reply continues in the customer’s existing web conversation.</p></div>{acceptState === "processing" ? <span className="status-chip bg-[#e6f8fd] text-[#078bb5]"><Loader2 className="h-3 w-3 animate-spin" />Preparing composer</span> : accepted ? <span className="status-chip bg-[#edf9f4] text-[#16734f]"><ShieldCheck className="h-3 w-3" />Agent verified</span> : <span className="status-chip bg-[#f1f5f9] text-slate-600">Accept hand-off to reply</span>}</div>{accepted ? <div className="space-y-4 p-4"><div className="grid gap-3 sm:grid-cols-[minmax(0,.72fr)_minmax(0,1.28fr)]"><label className="block"><span className="mb-1.5 block text-[10px] font-semibold uppercase tracking-[.075em] text-slate-500">Disposition code</span><select aria-label="Resolution disposition" value={disposition} onChange={(event) => setDisposition(event.target.value)} className="w-full rounded-lg border border-[#dce7f7] bg-white px-3 py-2.5 text-xs font-medium text-[#253b5a] outline-none focus:border-[#0866f5]"><option>Resolved — retry advised</option><option>Escalated — payment investigation</option><option>Customer action required</option><option>Information provided</option><option>No further action</option></select></label><label className="block"><span className="mb-1.5 block text-[10px] font-semibold uppercase tracking-[.075em] text-slate-500">Private agent notes</span><textarea aria-label="Private agent notes" value={agentNotes} onChange={(event) => setAgentNotes(event.target.value)} className="min-h-[68px] w-full resize-y rounded-lg border border-[#dce7f7] bg-[#fbfdff] p-2.5 text-xs leading-5 text-[#253b5a] outline-none focus:border-[#0866f5]" /></label></div><textarea aria-label="Human agent response" value={draft} onChange={(event) => setDraft(event.target.value)} className="min-h-[112px] w-full resize-y rounded-lg border border-[#dce7f7] bg-[#fbfdff] p-3 text-xs leading-5 text-[#253b5a] outline-none focus:border-[#0866f5]" /><div className="flex flex-wrap items-center justify-between gap-3"><p className="max-w-md text-[10px] leading-4 text-slate-500">The selected disposition and private note are stored with the case record. Safe guidance may be sent now; protected account actions remain unavailable until secure re-authentication is confirmed.</p><button onClick={sendReply} disabled={replySent} className="inline-flex items-center gap-2 rounded-lg bg-[#0866f5] px-3.5 py-2 text-xs font-semibold text-white hover:bg-[#075ddd] disabled:cursor-not-allowed disabled:opacity-55"><Send className="h-3.5 w-3.5" />{replySent ? "Reply sent" : "Send reply"}</button></div>{replySent && <div className="rounded-lg border border-[#bde8d3] bg-[#edf9f4] px-3 py-2 text-[11px] text-[#226e4d]">Response recorded with disposition: <strong>{disposition}</strong>. Private notes are included in the audit record.</div>}</div> : <div className="p-4 text-xs leading-5 text-slate-500">Accept this hand-off to identify yourself to the customer, enter a response, record a disposition, and take ownership of the support case.</div>}</section>
        </div>
      </article>
      <aside className="space-y-5 xl:pt-8"><section className="overflow-hidden rounded-xl border border-[#cfe4fa] bg-[#f4f8ff] shadow-[0_1px_2px_rgba(7,26,53,.025)]"><div className="border-b border-[#cfe4fa] bg-white/60 p-5"><div className="flex items-center gap-2"><img src={iconLogo} alt="FinCare AI" className="h-7 w-7" /><div><p className="text-xs font-bold text-[#071a35]">FinCare AI</p><p className="text-[10px] font-semibold text-[#078bb5]">● Operational AI context</p></div></div></div><dl className="space-y-0 p-5 text-xs"><div className="border-b border-[#dceafa] py-3"><dt className="text-slate-500">Detected intent</dt><dd className="mt-1 font-semibold text-[#071a35]">Transfer Failure</dd></div><div className="border-b border-[#dceafa] py-3"><dt className="text-slate-500">Confidence</dt><dd className="mt-1 text-lg font-bold tracking-[-.03em] text-[#0866f5]">96%</dd></div><div className="border-b border-[#dceafa] py-3"><dt className="text-slate-500">Response time</dt><dd className="mt-1 font-semibold text-[#071a35]">2.4s</dd></div><div className="border-b border-[#dceafa] py-3"><dt className="text-slate-500">Knowledge source</dt><dd className="mt-1 font-semibold text-[#071a35]">Account API</dd></div><div className="pt-3"><dt className="text-slate-500">Safety check</dt><dd className="mt-1"><Status value="Passed" /></dd></div></dl></section><section className="overflow-hidden rounded-xl border border-[#d6e7fa] bg-[#f4f8ff] p-5 shadow-[0_1px_2px_rgba(7,26,53,.02)]"><div className="flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#eaf3ff] text-[#0866f5]"><KeyRound className="h-4 w-4" /></span><div><p className="text-xs font-bold text-[#071a35]">Secure session</p><p className="text-[10px] font-medium text-[#078bb5]">Customer-controlled verification</p></div></div>{reauthState === "not-requested" && <><p className="mt-4 text-[11px] leading-5 text-[#506d92]">The human agent does not inherit the AI session. Account actions and protected details require a fresh customer step-up in Qorebank.</p><button onClick={requestReauth} className="mt-4 w-full rounded-lg bg-[#0866f5] px-3 py-2 text-xs font-semibold text-white hover:bg-[#075ddd]">Request secure re-authentication</button></>}{reauthState === "requested" && <><div className="mt-4 rounded-lg border border-[#bfe5f3] bg-white p-3"><p className="text-[11px] font-semibold text-[#078bb5]">Request sent to the customer</p><p className="mt-1 text-[10px] leading-4 text-slate-500">Verification happens in the existing bank app or browser. The agent never sees a password, PIN, or one-time code.</p></div><button onClick={() => setReauthState("verified")} className="mt-3 w-full rounded-lg border border-[#bfd6fb] bg-white px-3 py-2 text-xs font-semibold text-[#0866f5] hover:bg-[#eaf3ff]">Preview verified state</button></>}{reauthState === "verified" && <div className="mt-4 rounded-lg border border-[#bde8d3] bg-[#edf9f4] p-3"><p className="text-[11px] font-semibold text-[#16734f]">Secure session confirmed</p><p className="mt-1 text-[10px] leading-4 text-[#4d755f]">Protected account actions can now be initiated under the bank’s configured policy and audit controls.</p></div>}<p className="mt-4 text-[10px] leading-4 text-slate-500">Use re-authentication only when a policy-protected account action or sensitive customer data is needed—not simply because a human has joined the conversation.</p></section></aside>
    </section>
  </div>;
}
...
export default function Home() {
  ...
  const requestedPage = query.get("view") as PageKey | null;
  const initialPage = requestedPage && navigation.some((item) => item.label === requestedPage) ? requestedPage : "Overview";
  const [page, setPage] = useState<PageKey>(initialPage);
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const [conversationDetail, setConversationDetail] = useState(initialPage === "Conversations" && query.get("detail") === "true");
  ...
  const openConversationDetail = () => { setPage("Conversations"); setConversationDetail(true); window.history.replaceState(null, "", "/?view=Conversations&detail=true"); window.scrollTo({ top: 0, behavior: "smooth" }); };
  const closeConversationDetail = () => { setConversationDetail(false); window.history.replaceState(null, "", "/?view=Conversations"); window.scrollTo({ top: 0, behavior: "smooth" }); };
  return <div className="min-h-screen bg-[#f4f8ff] lg:flex">
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[260px] flex-col bg-[#071a35] transition-transform duration-200 lg:sticky lg:inset-y-auto lg:top-0 lg:h-screen lg:shrink-0 lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-5">{navigation.map((item) => { const Icon = item.icon; return <div key={item.label} className={item.group ? "mt-6 first:mt-0" : ""}>{item.group && <p className="mb-2 px-3 text-[10px] font-bold uppercase tracking-[.12em] text-[#7e95b9]">{item.group}</p>}<button onClick={() => choose(item.label)} className={`nav-item ${page === item.label ? "nav-item-active" : ""}`}><Icon className="h-4 w-4 shrink-0" /><span className="truncate">{item.label}</span>{item.label === "Escalations" && <span className="ml-auto rounded bg-white/[.13] px-1.5 py-0.5 text-[10px] text-white">18</span>}</button></div>; })}</div>
